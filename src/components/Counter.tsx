    ...
    return (
        <div className="flex-shrink-0 flex flex-col items-center justify-center w-full mt-4 lg:mt-0">
         ...
         `}>
            ...
            <div className={`text-center mt-8 inline-block px-5 py-2.5 rounded-full border backdrop-blur-md shadow-sm transition-colors duration-500 ${theme === 'dark' ? 'bg-white/5 border-white/10 text-slate-300' : 'bg-black/5 border-black/10 text-slate-700'} relative z-10`}>
                <span className="text-sm sm:text-base align-middle mr-1">You are currently running FullStacked on</span>
                <kbd className={`inline-block px-2.5 py-1 align-middle rounded-lg text-sm font-mono font-medium shadow-sm transition-colors duration-500 border ${theme === 'dark' ? 'bg-slate-800/80 text-fs-blue-400 border-white/10' : 'bg-white/80 text-fs-blue-600 border-black/5'}`}>
                    {platform}
                </kbd>
            </div>
        </div>
